...
class snapshot:
    '''takes two arguments path to the snapshot and the snapshot number'''
    def __init__(self, path, numSnapshot):
        
        d1 = readsnap(path, numSnapshot, 0)
        d2 = readsnap(path, numSnapshot, 1)
        
        d1['m'] = d1['m'].reshape((len(d1['m']),1))
        d2['m'] = d2['m'].reshape((len(d2['m']),1))
        ...
        dictionary = {'p': d1['p'], 'v': d1['v'], 'id': d1['id']}
        self.mGas = d1['m'][0][0]*1E10
        self.mDM = d2['m'][0][0]*1E10
        
        del d2
    
        rho = pd.DataFrame(d1['rho'], columns=['rho'])
        
        self.data = pd.DataFrame(dictionary)
        self.data = pd.concat([self.data, rho], axis=1)
        
        self.time = d1['time']
        self.numPart = d1['numPart']
        self.data.id -= 1
        
        self.data = self.data.sort_values(by='id', ignore_index=True)
        
        numPartFirst = int(d1['numPart'][0]/2)
        radius = np.max(np.linalg.norm(np.vstack(self.data.p.values[0:numPartFirst]), axis=1))
        self.radius = radius
        self.vc = np.sqrt((4.3E-3*(self.mGas+self.mDM)*np.sum(self.numPart[0:1])) / (self.radius*1E3))
        
        self.data.drop(['id'], axis=1, inplace=True)
        #print(self.data.memory_usage(index=True, deep=True).sum()*1E-6)
               
def snapshotListGenerator(path, step=10):
	'''Generate a list of snapshot objects at the given path of (2,n)'''
    numFile1 = len(glob.glob(path+'run1/snapshot_*'))
    numFile2 = len(glob.glob(path+'run2/snapshot_*'))
    
    snapshotList1, snapshotList2 = [], []
    
    for i in range(0, numFile1, step):
        snapshotList1.append(snapshot(path+'run1/', i))
        
    for i in range(0, numFile2, step):
        snapshotList2.append(snapshot(path+'run2/', i))

    return np.array([snapshotList1, snapshotList2])

def correlation(snapshotList, **kwargs):
    '''Calculates the spearman rank correlation between density and phase space separations'''
    start = time.time()
    ...
    for (snapi, snapj) in zip(run1, run2):

        timei.append(snapi.time)
        timej.append(snapj.time)

        densityi = snapi.data.rho[0:snapi.numPart[0]]
        densityj = snapj.data.rho[0:snapj.numPart[0]]
        
        if (snapi.time<0.0001):
            radius = snapi.radius
            vc = snapi.vc
            
        posSep = np.linalg.norm(np.vstack(snapi.data.p.values) - np.vstack(snapj.data.p.values), axis=1) / radius
        velSep = np.linalg.norm(np.vstack(snapi.data.v.values) - np.vstack(snapj.data.v.values), axis=1) / vc
        totSep = posSep + velSep

        Spearmani.append(spearmanr(densityi, posSep)[0])
        Spearmanj.append(spearmanr(densityj, posSep)[0])
        
        SpearmanVeli.append(spearmanr(densityi, velSep)[0])
        SpearmanVelj.append(spearmanr(densityj, velSep)[0])        
        
        SpearmanToti.append(spearmanr(densityi, totSep)[0])
        SpearmanTotj.append(spearmanr(densityj, totSep)[0])

    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True, figsize=(20,5))
    ax1.plot(timei, Spearmani, label='Spearman Run 1')
    ax1.plot(timej, Spearmanj, label='Spearman Run 2')
    
    ax2.plot(timei, SpearmanVeli, label='')
    ax2.plot(timej, SpearmanVelj, label='')
        
    ax3.plot(timei, SpearmanToti, label='')
    ax3.plot(timej, SpearmanTotj, label='')
    
    ax1.set_xlabel('Time [Gyr]')
    ax2.set_xlabel('Time [Gyr]')
    ax3.set_xlabel('Time [Gyr]')
    
    ax1.set_ylabel('Rank Coefficient')
    ax1.legend()
    ax1.set_title('Position Only')
    ax2.set_title('Velocity Only')
    ax3.set_title('Position+Velocity')

    end = time.time()
    print('Time taken to calculate Spearman/Pearson rank correlation:', end-start, 'seconds')
    
def phaseSpaceSeparation(path, numSnapshots, step=10):
	'''Takes as input the path to the directory where both the runs are stored, total number of snapshots in each run, 
	and the number of steps in which you want the snapshots to be read.'''

	#Initialize the array which stores the snapshot times
	time = []
	#Generate the directory phaseSpaceSeparations at this location.
	pathlib.Path(path+'phaseSpaceSeparations').mkdir(parents=True, exist_ok=True) 
	#Initialize position, velocity and total separation arrays which will store the cumulative separations at each time
	positions, velocities, total = deque(), deque(), deque()
	for i in range(0, numSnapshots+1, step):
	
		#read the two corresponding snapshots from folders run1 and run2
		snapi = snapshot(path+'run1/', i)
		snapj = snapshot(path+'run2/', i)

		#append the time of snapshot to the time array
		time.append(snapi.time)

		#set the scaling for positions and velocities according to the radius of one halo at t=0, and the circular velocity
		#at the radius respectively
		if i==0:
			radius = snapi.radius
			vc = snapi.vc

		#create p, v, and total separation arrays which represent the separations of every particle
		posSep = np.linalg.norm(np.vstack(snapi.data.p.values) - np.vstack(snapj.data.p.values), axis=1) / radius
		velSep = np.linalg.norm(np.vstack(snapi.data.v.values) - np.vstack(snapj.data.v.values), axis=1) / vc
		totSep = posSep + velSep
		
		#sum the separations of every particle and append it to the cumulative separation array
		positions.append(np.sum(posSep))
		velocities.append(np.sum(velSep))
		total.append(np.sum(totSep))
		
		#check if the separation file already exists and if not, dump separation data for each snapshot onto a file.
		try:
			f = open(path+'phaseSpaceSeparations/positionSeparationArray{}Myr'.format(int(snapi.time*1000)))
			f.close()
		except:
			np.savetxt(path+'phaseSpaceSeparations/positionSeparationArray{}Myr'.format(int(snapi.time*1000)), np.insert(posSep, 0, snapi.numPart[0]), delimiter='\n', encoding='ASCII', fmt='%f')
			np.savetxt(path+'phaseSpaceSeparations/velocitySeparationArray{}Myr'.format(int(snapi.time*1000)), np.insert(velSep, 0, snapi.numPart[0]), delimiter='\n', encoding='ASCII', fmt='%f')
			np.savetxt(path+'phaseSpaceSeparations/totalSeparationArray{}Myr'.format(int(snapi.time*1000)), np.insert(totSep, 0, snapi.numPart[0]), delimiter='\n', encoding='ASCII', fmt='%f')
		
		#delete snapshot objects
		del snapi, snapj		
	
	#return the time array, and cumulative separations
	return np.array(time), positions, velocities, total

####################################################################################################################################
#Functions from here on are written by Phil Hopkins and the Gizmo team. ############################################################
####################################################################################################################################
def readsnap(sdir,snum,ptype,
    ...
    if(fname_ext=='.hdf5'):
		...
		## In the open statement below, I added 'rb' to indicate readonly binary. It wasn't there before and the compiler threw an error
        file = open(fname, 'rb') # Open binary snapshot file
    ...
    file.close();
    if (ptype==0):
        return {'numPart':npartTotal, 'time':time, 'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'ne':nume,'nh':numh,'sfr':sfr,'z':metal};
    if (ptype==4):
        return {'numPart':npartTotal, 'time':time, 'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'z':metal,'age':stellage}
    if (ptype==5) and (skip_bh==0):
        return {'numPart':npartTotal, 'time':time, 'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'mbh':bhmass,'mdot':bhmdot}
...
def load_gadget_format_binary_particledat(f, header, ptype, skip_bh=0):
	...
	################################################################################################################################
	## Block of code added by Paresh Mungara to skip two records which are just lists of zeros #####################################
	################################################################################################################################
    skip = array.array('f')
    skip.fromfile(f, NpartTot)
    skip = np.array(skip)
    f.read(4+4) # Read block size fields.
    ...
    Npart_MassCode[(Npart <= 0) | (np.array(Massarr,dtype='d') > 0.0)] = np.int32(0)
